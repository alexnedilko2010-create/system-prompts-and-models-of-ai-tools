...
exports.sendTransactions = exports.isVersionedTransaction = exports.TransactionBuilder = exports.getSizeOfCompressedU16 = exports.getSizeOfTransaction = exports.DEFAULT_PRIORITY_FEE_CONFIG = exports.JITO_BUNDLE_SIZE = exports.PACKET_DATA_SIZE_WITH_ROOM_FOR_COMPUTE_BUDGET = exports.DEFAULT_COMPUTE_BUDGET_UNITS = void 0;
...
class TransactionBuilder {
    ...
    constructor(payer, connection, addressLookupTable) {
        this.payer = payer;
    ...
    addInstruction(args) {
        ...
        if (this.transactionInstructions.length === 0) {
            ...
            const sizeWithComputeUnits = getSizeOfTransaction([
                ...this.transactionInstructions[this.transactionInstructions.length - 1].instructions,
                instruction,
                this.transactionInstructions.length % exports.JITO_BUNDLE_SIZE === 0 // This transaction may be the first of a Jito bundle, so we leave room for a Jito tip transfer.
                    ? (0, jito_1.buildJitoTipInstruction)(this.payer, 1)
                    : web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({
                        microLamports: 1,
                    }),
                web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 1 }),
            ], true, this.addressLookupTable);
            if (sizeWithComputeUnits <= web3_js_1.PACKET_DATA_SIZE) {
                this.transactionInstructions[this.transactionInstructions.length - 1].instructions.push(instruction);
                this.transactionInstructions[this.transactionInstructions.length - 1].signers.push(...signers);
                this.transactionInstructions[this.transactionInstructions.length - 1].computeUnits += computeUnits ?? 0;
            }
            else
                this.transactionInstructions.push({
                    instructions: [instruction],
    ...
    async buildVersionedTransactions(args) {
        const blockhash = (await this.connection.getLatestBlockhash({ commitment: "confirmed" })).blockhash;
        return this.transactionInstructions.map(({ instructions, signers, computeUnits }, index) => {
            const instructionsWithComputeBudget = [
                ...instructions,
            ];
            if (computeUnits > exports.DEFAULT_COMPUTE_BUDGET_UNITS * instructions.length ||
                args.tightComputeBudget) {
                instructionsWithComputeBudget.push(web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: computeUnits }));
            }
            if (args.computeUnitPriceMicroLamports) {
                instructionsWithComputeBudget.push(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({
                    microLamports: args.computeUnitPriceMicroLamports,
                }));
            }
            if (args.jitoTipLamports && index % exports.JITO_BUNDLE_SIZE === 0) {
                instructionsWithComputeBudget.push((0, jito_1.buildJitoTipInstruction)(this.payer, args.jitoTipLamports));
            }
            ...
            for (const instruction of instructionsWithComputeBudget) {
                const sizeWithInstruction = getSizeOfTransaction([...instructionsToSend, instruction], true, this.addressLookupTable);
                if (sizeWithInstruction > web3_js_1.PACKET_DATA_SIZE) {
                    break;
                }
                instructionsToSend.push(instruction);
            }
            return {
                tx: new web3_js_1.VersionedTransaction(new web3_js_1.TransactionMessage({
                    recentBlockhash: blockhash,
                    instructions: instructionsToSend,
                    payerKey: this.payer,
                }).compileToV0Message(this.addressLookupTable ? [this.addressLookupTable] : [])),
                signers: signers,
            };
        });
    }
    ...
    buildLegacyTransactions(args) {
        return this.transactionInstructions.map(({ instructions, signers, computeUnits }, index) => {
            ...
            const instructionsToSend = [];
            for (const instruction of instructionsWithComputeBudget) {
                const sizeWithInstruction = getSizeOfTransaction([...instructionsToSend, instruction], false);
                if (sizeWithInstruction > web3_js_1.PACKET_DATA_SIZE) {
                    if (instructionsToSend.length == 0) {
                        throw new Error(`An instruction is too big to be sent in a transaction (${sizeWithInstruction} > ${web3_js_1.PACKET_DATA_SIZE} bytes)`);
                    }
                    break;
                }
                instructionsToSend.push(instruction);
            ...
            return {
                tx: new web3_js_1.Transaction().add(...instructionsToSend),
                signers: signers,
            };
        });
    }
    ...
    static batchIntoLegacyTransactions(instructions, priorityFeeConfig) {
        const transactionBuilder = new TransactionBuilder(web3_js_1.PublicKey.unique(), new web3_js_1.Connection("http://placeholder.placeholder")); // We only need wallet and connection for `VersionedTransaction` so we can put placeholders here
...
async function sendTransactions(transactions, connection, wallet, maxRetries) {
    ...
    for (const [index, transaction] of transactions.entries()) {
        ...
        const txSignature = bs58_1.default.encode((0, exports.isVersionedTransaction)(tx)
